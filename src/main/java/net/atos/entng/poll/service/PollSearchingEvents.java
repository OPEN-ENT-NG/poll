...
public class PollSearchingEvents implements SearchingEvents {

	private static final Logger log = LoggerFactory.getLogger(PollSearchingEvents.class);
	private SearchService searchService;

	public PollSearchingEvents(SearchService searchService) {
	...
	public void searchResource(List<String> appFilters, String userId, JsonArray groupIds, JsonArray searchWords, Integer page, Integer limit,
		...
		searchFields.add("question");


		if (appFilters.contains(PollSearchingEvents.class.getSimpleName())) {
			searchService.search(userId, groupIds.toList(), returnFields, searchWords.toList(), searchFields, page, limit, new Handler<Either<String, JsonArray>>() {
				...
				public void handle(Either<String, JsonArray> event) {
					...
					if (log.isDebugEnabled()) {
						log.debug("[PollSearchingEvents][searchResource] The resources searched by user are finded");
					}
				}
			});
		} else {
	...
	private JsonArray formatSearchResult(final JsonArray results, final JsonArray columnsHeader) {
		...
		for (int i=0;i<results.size();i++) {
			...
			if (j != null) {
				jr.putString(aHeader.get(0), j.getString("question"));
				jr.putString(aHeader.get(1), "");
				...
				jr.putString(aHeader.get(5), "/poll#/view/" + j.getString("_id"));
				traity.add(jr);
			}
		}
		return traity;
